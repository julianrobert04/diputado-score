    ...
    <div className="min-h-screen bg-[#0c0c0e] text-white">
      ...
      <header className="sticky top-0 z-50 bg-[#0c0c0e]/80 backdrop-blur-xl border-b border-white/[0.05]">
        <div className="max-w-5xl mx-auto px-5 h-14 flex items-center justify-between gap-4">
          ...
          <Link href="/" className="flex items-center gap-1.5 text-zinc-500 hover:text-white text-sm transition-colors">
      ...
      <main className="max-w-5xl mx-auto px-5 pt-8 pb-16">
          ...
          <div className="relative p-7 sm:p-10">
            <div className="flex flex-col sm:flex-row items-start gap-8">
              ...
              <div className="flex-1 min-w-0">
                ...
                <p className="text-zinc-500 text-sm sm:hidden mb-1">{politicianParty}</p>
                <div className="flex flex-wrap items-center gap-x-4 gap-y-1 mt-2">
                  ...
                  {periodStart && (
                    <span className="text-zinc-600 text-sm">
                      {new Date(periodStart).getFullYear()}–{periodEnd ? new Date(periodEnd).getFullYear() : ""}
                    </span>
                  )}
                  {!politicianActive && (
                ...
                {metrics && (
                  <div className="flex flex-wrap gap-2 mt-4">
                    {Object.entries(DIMENSION_META).map(([key, dim]) => {
                      ...
                      return (
                        <div key={key} className="flex items-center gap-1.5 bg-zinc-800/70 rounded-lg px-2.5 py-1.5 ring-1 ring-white/[0.05]">
                          <span className={`${SCORE_TEXT[dimColor]} text-sm font-black tabular-nums`}>{dimScore.toFixed(1)}</span>
                          <span className="text-zinc-500 text-[0.65rem]">{dim.label}</span>
                        </div>
                      );
                    })}
                  </div>
              ...
              <div className="sm:flex-shrink-0 flex sm:flex-col items-center gap-3 sm:gap-1 sm:items-end">
                <div className={`${SCORE_TEXT[color]} text-[4.5rem] sm:text-[5.5rem] font-black tabular-nums leading-none tracking-tight`}>
                  {overall.toFixed(1)}
                </div>
                <div className="flex items-center gap-2 sm:flex-col sm:items-end sm:gap-1">
                  ...
                  {latestSnapshot && (
                    <span className="text-[0.65rem] text-zinc-600">
                      {new Date(latestSnapshot.takenAt).toLocaleDateString("es-CR", { day: "numeric", month: "short" })}
                    </span>
                  )}
                </div>
              </div>
          ...
          <div className="bg-zinc-900 rounded-2xl ring-1 ring-white/[0.06] p-6 mb-5">
            ...
            <div className="grid grid-cols-2 sm:grid-cols-3 gap-2.5">
              {[
                ...
                { label: "Votaciones", value: `${rawData.votacionesAsistidas ?? "—"} / ${rawData.votacionesTotales ?? "—"}` },
        ...
        {allPeriods.length > 1 && (
          <div className="bg-zinc-900 rounded-2xl ring-1 ring-white/[0.06] p-6">
            <h2 className="text-base font-bold mb-4 text-white">Historial de períodos</h2>
            <div className="space-y-2">
              {allPeriods.map((period) => (
                <div key={period.id} className="flex items-center justify-between bg-zinc-800/60 rounded-xl px-4 py-3 ring-1 ring-white/[0.04]">
                  <div>
                    <p className="text-sm font-semibold text-white">
                      {new Date(period.startDate).getFullYear()} – {new Date(period.endDate).getFullYear()}
                    </p>
                    <p className="text-xs text-zinc-600">{period.party}</p>
                  </div>
                  {period.score && (
                    <span className={`${SCORE_TEXT[getScoreColor(period.score.overall)]} text-lg font-black tabular-nums`}>
