...
const STAT_TILE =
  "bg-zinc-900 ring-1 ring-white/[0.06] rounded-xl px-4 py-3 hover:ring-white/[0.12] transition-all duration-200";
  ...
  return (
    <div className="min-h-screen bg-[#0c0c0e] text-white">
      ...
      <main className="max-w-7xl mx-auto px-5 py-12">
        ...
        <div className="relative mb-12">
          ...
          <div
            ...
            style={{
              background:
                "radial-gradient(560px 280px at 22% 20%, rgba(16,185,129,0.08), transparent 70%)",
            }}
          />
            ...
            <div className="flex-1 min-w-0">
              ...
              <div className="mb-7 border-l-2 border-emerald-500/40 pl-4">
                <p className="text-zinc-400 text-sm italic leading-relaxed">
                  &quot;Un país que le exige más a un futbolista que a un
                  político
                  <br className="hidden sm:block" /> está condenado a la
                  mediocridad.&quot;
                </p>
                <span className="text-zinc-300 text-xs mt-1 block">
                  — Anónimo
                </span>
              </div>

              <div className="flex flex-wrap items-center gap-2 mb-5">
                <span className="text-zinc-300 text-[0.7rem]">
                  Actualizado{" "}
                  {new Date(REAL_DATA_INFO.updatedAt).toLocaleDateString(
                    "es-CR",
                    { day: "numeric", month: "long", year: "numeric" },
                  )}{" "}
                  · datos abiertos de la Asamblea
                </span>
              </div>
              <h1 className="text-3xl sm:text-[2.75rem] font-black tracking-tight leading-[1.1] mb-4">
                ...
                <span className="text-emerald-400">
                  que el de nuestros diputados.
                </span>
              </h1>
              <p className="text-zinc-500 text-base max-w-xl leading-relaxed text-pretty">
                Inspirados en los puntajes con los que se evalúa a los
                jugadores, calificamos el desempeño de cada diputado de la
                ...
                <div className="bg-zinc-900/80 backdrop-blur rounded-2xl ring-1 ring-white/[0.06] p-5">
                  ...
                  <div className="space-y-2">
                    {top3.map(({ card }, i) => (
                      ...
                      >
                        <span
                          className={`${MEDAL[i]} text-sm font-black tabular-nums w-4 flex-shrink-0`}
                        >
                          {i + 1}
                        </span>
                        <RankingAvatar
                        ...
                        <div className="flex-1 min-w-0">
                          ...
                          <p className="text-zinc-300 text-[0.62rem] truncate">
                            {PARTIDO_SHORT[card.party] ?? card.party}
                          </p>
                        </div>
                        <span
                          className={`${SCORE_TEXT[getScoreColor(card.overall)]} text-sm font-black tabular-nums`}
                        >
        ...
        <div className="grid grid-cols-2 sm:grid-cols-4 gap-3 mb-10">
          <div className={STAT_TILE}>
            <p className="text-zinc-300 text-[0.65rem] font-semibold uppercase tracking-widest mb-1">
              Score promedio
            </p>
            <p
              className={`text-xl font-black ${SCORE_TEXT[getScoreColor(avgScore)]}`}
            >
              {avgScore.toFixed(1)}
            </p>
          </div>
            ...
            {best ? (
              <>
                <p className="text-white text-sm font-bold leading-tight truncate">
                  {shortName(best.card.fullName)}
                </p>
                <p className="text-zinc-500 text-xs">
                  {best.card.overall.toFixed(1)}
                </p>
              </>
            ) : (
            ...
            {bestParty ? (
              <>
                <p className="text-white text-sm font-bold">
                  {PARTIDO_SHORT[bestParty.name] ?? bestParty.name}
                </p>
                <p className="text-zinc-500 text-xs">
                  {bestParty.avg.toFixed(1)} prom.
                </p>
              </>
            ) : (
              <p className="text-zinc-400 text-sm">—</p>
          ...
          {politicians.some((p) => p.latestDelta !== null) && (
            <div className="flex items-center gap-3 text-[0.68rem] text-zinc-300">
              <span className="flex items-center gap-1">
                <span className="text-emerald-400">▲</span> subió
              </span>
              <span className="flex items-center gap-1">
                <span className="text-rose-400">▼</span> bajó
              </span>
        ...
        {politicians.length === 0 ? (
          <div className="text-center py-32 text-zinc-400">
            ...
            <p className="text-sm mt-2 text-zinc-400">
              Corré{" "}
              <code className="bg-zinc-900 px-1.5 py-0.5 rounded text-zinc-500">
                npm run ingest:opendata
              </code>{" "}
              para cargar datos
            </p>
          </div>
      ...
      <footer className="mt-20 border-t border-white/[0.04] py-8">
        <div className="max-w-7xl mx-auto px-5 flex flex-col sm:flex-row items-center justify-between gap-2 text-zinc-400 text-xs">
          <p>
