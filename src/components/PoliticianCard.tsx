...
const CARD_METRICS: Array<{
  key: keyof PoliticianCardType["metrics"];
  label: string;
}> = [
    ...
    <Link href={`/diputados/${politician.id}`} className="block group">
      ...
      >
          ...
          <div className="relative">
            ...
            >
              {politician.photoUrl && !imgError ? (
                // `unoptimized`: los retratos vienen de www.asamblea.go.cr, cuyo
                // servidor omite el CA intermedio de su cadena TLS. El optimizador
                // de imágenes de Next no puede verificar esa cadena y fallaría la
                // descarga, sustituyendo cada foto por el fallback de la inicial.
                // Servimos el original sin optimizar para que las fotos se vean.
                <Image
        ...
        <div className="px-3 pt-4 pb-2.5 text-center">
          ...
          <p className="text-zinc-300 text-[0.6rem] mt-0.5 truncate leading-tight">
            {politician.party}
          </p>
          <div className="flex items-center justify-center gap-1 mt-0.5">
            <svg
              className="w-2 h-2 text-zinc-700 flex-shrink-0"
              fill="currentColor"
              viewBox="0 0 24 24"
            >
            ...
            {!politician.active && (
              <span className="text-[0.55rem] bg-zinc-800/80 text-zinc-300 px-1.5 py-0.5 rounded-full border border-zinc-700/40">
                Histórico
              </span>
            )}
          </div>
